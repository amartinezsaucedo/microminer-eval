...
import microminer_eval.algorithm.Utils as utils
import networkx as nx
import javalang
import logging
import numpy as np
import microminer_eval.algorithm.LDA as lda
import argparse
import json
import re
import os
from microminer_eval.algorithm.Graph import Graph
from microminer_eval.algorithm.TfIdf import TfIdf
from microminer_eval.algorithm.WeightType import WeightType
from microminer_eval.algorithm.Clustering import Clustering
from microminer_eval.algorithm.visitors.ClassVisitor import ClassVisitor
from microminer_eval.algorithm.entities.Method import Method
from microminer_eval.algorithm.entities.Class import Class
from microminer_eval.algorithm.Settings import Settings
...
def partition(args):
    Settings.DRAW = True if args.get("draw") else False
    Settings.LDA_PLOTTING = True if args.get("lda_plotting") else False
    Settings.K_TOPICS = int(args.get("k_topics")) if args.get("k_topics") else None
    Settings.RESOLUTION = float(args.get("resolution")) if args.get("resolution") else None

    Settings.set_stop_words(args.get("stop_words"))

    print(f"Setting Directory as: {Settings.DIRECTORY}")

    if args.get("project"):
        project_name = str(args.get("project")).split('/')[-1]
        project_path = str(args.get("project"))
        Settings.PROJECT_PATH = project_path
        Settings.PROJECT_NAME = project_name

        return identify_clusters_in_project(project_name, project_path)
